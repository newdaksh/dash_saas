...
import { X, Calendar, CheckCircle2, AlertCircle, MessageSquare, Send, ChevronDown, Building2, Crown, Trash2, AlertTriangle, Save, Briefcase, Users, History, Clock, ArrowRight, Edit3, UserPlus, Flag, FileText, Layers, Eye } from 'lucide-react';
...
interface TaskPanelProps {
  ...
  readOnly?: boolean; // When true, NO changes are allowed (strict view only)
}

export const TaskPanel: React.FC<TaskPanelProps> = ({ task, isOpen, onClose, viewOnly = false, readOnly = false }) => {
  ...
  const handleStatusChange = async () => {
    if (readOnly) return; // Strict read-only check
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-100">
          <Button
            ...
            className={`flex items-center gap-2 ${localTask.status === Status.DONE ? 'text-green-600 bg-green-50' : ''} ${readOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
          ...
          <div className="flex items-center gap-2">
            {/* Save/Discard buttons when there are changes - hidden in view-only and read-only mode */}
            {!viewOnly && !readOnly && (
              <>
                {hasChanges && (
                  <Button
