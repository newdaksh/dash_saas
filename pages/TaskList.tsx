...
const TaskTitleInput: React.FC<{
  ...
  readOnly?: boolean;
}> = React.memo(({ task, onUpdate, onKeyDown, readOnly }) => {
  ...
  return (
    <input
      ...
      autoComplete="off"
      readOnly={readOnly}
      className={`w-full bg-transparent border border-transparent rounded px-2 -ml-2 py-1.5 -my-1 text-sm md:text-base font-semibold block transition-all placeholder-slate-400 ${readOnly ? 'cursor-default focus:outline-none' : 'focus:bg-white focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10'
                  ...
                  >
                    ...
                    <div className="col-span-12 md:col-span-5 flex items-center gap-4 relative z-10">
                      <div
                        ...
                        className={`transition-all duration-300 group-hover:scale-110 p-1 rounded-full cursor-pointer z-20 ${task.status === Status.DONE
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <TaskTitleInput
                          ...
                          readOnly={task.creator_id !== user?.id}
                        />

                        <div className="flex items-center gap-2 mt-1 md:hidden">
                          <span className={`text-[10px] px-1.5 py-0.5 rounded border ${task.priority === Priority.HIGH ? 'bg-red-50 text-red-600 border-red-100' :
                    ...
                    <div className="hidden md:flex col-span-1 text-right justify-end items-center relative z-10">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border shadow-sm transition-transform group-hover:scale-105 ${task.priority === Priority.HIGH ? 'bg-red-50/80 text-red-600 border-red-100' :
